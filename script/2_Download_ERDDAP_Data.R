...
uP = unique(ParamDF$PARAMETER.NAME); uP
...
for (iP in 1:length(uP)){ 
  ...
  if (thisp$FREQUENCY == "Climatology"){
    ...
    for (ii in 1:length(uI)){
      ...
      # need to make sure time difference is bigger than timestep, otherwise the timestep runs backward...
      # K.T - Nov 4, 2021
      if (as.numeric(block_step) < TIMESTEP) {
        
        Nblocks = ceiling((singlestep * thisp$TIMESTEPS) / (thisp$BLOCKSIZE+50000)); Nblocks
        block_step = ((ts_end - ts_start)) / Nblocks
        block_step; Nblocks
